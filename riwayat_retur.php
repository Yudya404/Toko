					...
					<div class="card mb-3 mb-xl-10">
						<div class="card-body pt-9 pb-0">
							...
							<ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bold">
								...
								<!--begin::Nav item-->
								<li class="nav-item mt-2">
									<a class="nav-link text-active-primary ms-0 me-10 py-1" href="riwayat_retur.php">Riwayat Retur</a>
								</li>
								<!--end::Nav item-->
							</ul>
							<!--begin::Navs-->
					...
					<div class="card mb-5 mb-xl-10">
						...
						<div class="card-header cursor-pointer">
							...
							<div class="card-title m-0">
								<h3 class="fw-bold m-0">Riwayat Retur</h3>
							</div>
							<!--end::Card title-->
						</div>
						<!--begin::Card header-->
									...
									<tbody class="text-gray-600 fw-semibold">
										<?php
											if (isset($_SESSION['kode_cus'])) {
												...
												$query = "SELECT t.*, c.nama AS nama, c.foto_cus AS foto_cus FROM t_retur t
														  JOIN customer c ON t.kode_cus = c.kode_cus 
														  WHERE c.kode_cus = '$kode_cus'
														  ORDER BY t.kode_r DESC
														  LIMIT $offset, $itemsPerPage";
												$result = mysqli_query($conn, $query);
												
												// Hitung jumlah total data
												$result_total = mysqli_query($conn, "SELECT COUNT(*) as total FROM t_retur t
																					  JOIN customer c ON t.kode_cus = c.kode_cus 
																					  WHERE c.kode_cus = '$kode_cus'");
												...
												while ($row = mysqli_fetch_assoc($result)) {
													...
													$detailRetur = json_decode($row['detail_retur'], true);
										?>
											<tr>
												...
												<td class="text-center">
													<?php
														...
														if ($status == 'Pengajuan Retur Selesai') {
															$statusClass = 'text-success';
														} elseif ($status == 'Pengajuan Retur Disetujui') {
															$statusClass = 'text-success';
														} elseif ($status == 'Pengajuan Retur Diproses') {
															$statusClass = 'text-warning';
														} elseif ($status == 'Pesanan Menunggu Diproses') {
															$statusClass = 'text-warning';
														} elseif ($status == 'Pengajuan Retur Dibuat') {
															$statusClass = 'text-secondary';
														} elseif ($status == 'Pengajuan Retur Dibatalkan') {
												...
												<td class="text-center">
													...
													<div class="menu-item px-2">
														<div class="btn-group" role="group">
															<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#statusModal_<?= $row['kode_r'];  ?>"><i class="fa fa-eye"></i></button>
															<button type="button" class="btn btn-success" data-toggle="modal" data-target="#invoiceModal_<?= $row['kode_r'];  ?> <?= $isDisable ? 'disabled' : ''; ?>"><i class="fa fa-print"></i></button>
														</div>
													</div>
													<!--end::Menu item-->
												</td>
													...
													<table>
														<div class="col-lg-12">
															<div class="checkout__order">
																<p>Kamu belum memiliki riwayat retur.</p>
															</div>
														</div>
													</table>
												</div>
