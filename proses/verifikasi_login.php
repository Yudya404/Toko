    ...
    if ($statement) {
        ...
        if ($result) {
            ...
            if ($user && password_verify($password, $user['password'])) {
                // Verifikasi sukses
                $_SESSION['kode_cus'] = $user['kode_cus'];
				$_SESSION['nama'] = $user['nama'];
				$_SESSION['no_telp'] = $user['no_telp'];// Atur sesi nama pengguna
				$_SESSION['email'] = $user['email'];
				$_SESSION['alamat'] = $user['alamat'];
				$_SESSION['tgl_join'] = $user['tgl_join'];
				$_SESSION['foto_cus'] = $user['foto_cus'];

                // Redirect ke halaman sesuai peran pengguna (misalnya admin atau user)
                header('Location: ../index.php'); // Ganti dengan halaman beranda
                exit();
            } else {
                // Verifikasi gagal
				$_SESSION['error_message'] = 'Login gagal. Silakan periksa kembali email dan password Anda.';
